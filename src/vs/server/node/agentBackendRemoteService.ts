...
import { OpenCodeProcessManager, OpenCodeProcessState } from '../../platform/agentMode/node/opencodeProcessManager.js';

/**
 * Server-side agent backend service that runs on the remote host (WSL/SSH).
 * Spawns and manages the opencode subprocess locally on the remote machine,
 * mirroring the behaviour of `AgentBackendMainService` without Electron dependencies.
 */
export class AgentBackendRemoteService extends Disposable implements IAgentBackendService {
	...
	get location(): AgentBackendLocation {
		return AgentBackendLocation.Remote;
	}
	...
	private async _consumeEventStream(baseUrl: string, abortController: AbortController): Promise<void> {
		try {
			...
			const parser = new SSEParser(event => {
				...
				try {
					...
					if (typeof parsed !== 'object' || parsed === null) {
						this._logService.trace('[AgentMode][Remote] Ignoring non-object OpenCode SSE payload', parsed);
						return;
					}
					...
					this._logService.trace('[AgentMode][Remote] Failed to parse OpenCode SSE payload', error);
				}
			});
			...
			this._logService.warn('[AgentMode][Remote] OpenCode SSE stream disconnected', error);
		} finally {
			if (this._eventStreamLifecycle.value) {
				this._eventStreamLifecycle.clear();
			}
	...
	private _getRuntimeMetadata(): IAgentBackendRuntimeMetadata {
		return {
			location: AgentBackendLocation.Remote,
			requestedLocation: AgentBackendLocation.Remote,
